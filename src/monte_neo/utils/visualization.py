...
Requires optional plot extras: ``pip install "monte-neo[plot]"``.
"""
...
def _require_plot():
    try:
        ...
        raise ImportError(
            'Plotting requires optional deps. Install with: pip install "monte-neo[plot]"'
        ) from e
...
def plot_drawdown_dist(results: list[dict[str, Any]], title: str = "Max Drawdown Distribution"):
    ...
    plt.axvline(np.mean(drawdowns), color="r", linestyle="--", label=f"Mean: {np.mean(drawdowns):.2%}")
    plt.title(title)
...
plot_drawdown_distribution = plot_drawdown_dist  # alias
...
def plot_stress_test_summary(stress_results: dict[str, Any]):
    ...
    if bp:
        ...
        names.append("Break Pt (bps)/100")
        values.append(float(bp_val) / 100.0)
    plt.bar(names, values, color=["red", "blue", "green"][: len(names)])
